#!/usr/bin/env python3
"""
Airtable MCP Server for FastMCP 2.x
Compatible with ChatGPT and governance.
...
# ---------------------------------------------------------
# Helper: Airtable HTTP client using async httpx
# ---------------------------------------------------------
async def api_call(endpoint: str, method: str = "GET",
    ...
    headers = {"Authorization": f"Bearer {server_state['token']}",
...
# ---------------------------------------------------------
server = FastMCP("Airtable MCP Server")  # required name
...
async def update_records(table_name: str, records_json: Any):
    ...
    try:
        ...
        if not isinstance(records_data, list):
            return "records_json must be a list of records"
        ...
        for rec in records_data:
            ...
            fields = rec.get("fields", {k: v for k, v in rec.items() if k != "id"})
            final_records.append({"id": rec_id, "fields": fields})
...
async def delete_records(table_name: str, record_ids: Any):
    ...
    try:
        if isinstance(record_ids, str):
            record_ids = json.loads(record_ids) if record_ids.strip().startswith("[") else [
                rid.strip() for rid in record_ids.split(",")
            ]

        ids = list(record_ids)
        deleted_count = 0

        # Airtable allows deleting max 10 records per request
